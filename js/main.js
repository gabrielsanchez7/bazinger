function App(){
    ...
    this.contTestimonials = $('.rotate-tests');
...
App.prototype.handler = function(){
    ...
    for(i = 1; i <= arrays.testimonials.length; i++){
        $('.cont-handler-test').append('<span class="handler-test handler' + i + '"></span>');
    }
...
App.prototype.slideAuto = function(){
    var ob = this;
    if(ob.positionSlide <= ob.nSlides){
        $('.slide-active').removeClass('slide-active').next().addClass('slide-active');
        ob.positionSlide++;
        if(ob.positionSlide === (ob.nSlides)){
            $('.slide1').addClass('slide-active');
            $('.slide1').siblings().removeClass('slide-active');
            ob.positionSlide = 1;
        }
    }
    ob.changeBg();
    ...
    $('.screen-shot').click(function(ev){
        for(i = 0; i < arrays.gallery.length; i++){
            ...
            if($(target).hasClass('screenShot-' + idPop)){
                $('.popImg-' + idPop).addClass('pop-img-active').siblings().removeClass('pop-img-active');
            }
        }
    });
}
...
App.prototype.rotateTest = function(){
    var ob = this;
    setInterval(function(){
        $('.handler-test-active').removeClass('handler-test-active')
            .next()
            .addClass('handler-test-active')
            .siblings()
            .removeClass('handler-test-active');
        ob.positionSlide++;
        if(ob.positionSlide > (ob.nTests + 1)) {
            $('.handler1').addClass('handler-test-active')
                .siblings()
                .removeClass('handler-test-active');
            ob.positionSlide = 1;
        }
        if($('.handler1').hasClass('handler-test-active')){
            ob.contTestimonials.css({'-webkit-transform': 'translateX(0)'});
            $('.handler-test').css('background-color', 'rgba(52, 143, 180, 0.5)');
            $('.handler-test-active').css('background-color', '#348fb4');
        }
        else if($('.handler2').hasClass('handler-test-active')){
            ob.contTestimonials.css({'-webkit-transform': 'translateX(-800px)'});
            $('.handler-test').css('background-color', 'rgba(101, 139, 72, 0.5)');
            $('.handler-test-active').css('background-color', '#658b48');
        }
        else if($('.handler3').hasClass('handler-test-active')){
            ob.contTestimonials.css({'-webkit-transform': 'translateX(-1600px)'});
            $('.handler-test').css('background-color', 'rgba(188, 92, 0, 0.5)');
            $('.handler-test-active').css('background-color', '#bc5c00');
        }
    }, 5000);
    $('.handler1').click(function(){
        ob.contTestimonials.css({'-webkit-transform': 'translateX(0)'});
        $(this).addClass('handler-test-active').siblings().removeClass('handler-test-active');
        $('.handler-test').css('background-color', 'rgba(52, 143, 180, 0.5)');
        $('.handler-test-active').css('background-color', '#348fb4');
    });
